...
public class PublishMessageCommandImpl extends CommandImpl<MessageEvent>
  ...
  public PublishMessageCommandImpl(
      RequestManager commandManager,
      ZeebeObjectMapperImpl objectMapper,
      String topic,
      ClientTopicsMetada topicsMetadata) {
  ...
  public ResponseFuture<MessageEvent> send() {
    ...
    if (topicMetadata == null) {
      final String errorMessage =
          String.format("Cannot publish message to topic '%s', no such topic exists.", topic);
      throw new RuntimeException(errorMessage);
    } else {
      final String messageKey = command.getMessageKey(); // TODO null check
      final MessageDigest digest = getHashProvider();
